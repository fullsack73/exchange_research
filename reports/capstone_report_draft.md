...
작성일: 2026년 5월 18일  
작성 기준: 프로젝트 분석 산출물 및 전처리 데이터 검토
...
| 단기 유동성 | 일별 MMF와 보조적 CMA 관련 지표 | 일별 | 임계점 회귀, SHAP dependence, 예측 모델 검증 |
...
네 번째 목적은 월별 거시·금융 확장 데이터셋을 구성하는 것이다. 이 자료는 원/달러 환율뿐 아니라 수출입, 무역수지, 물가, 산업생산, 금융시장 변수, 외국인 투자, 심리 지표 등을 월별 기준으로 연결한 통합 자료이다. 환율 변동이 이후 국내 경제 변수에 어떤 영향을 미치는지 살펴보는 확장 분석에서 사용되며, 변수별 변화율과 시차 변수를 포함해 동태적 반응을 추정할 수 있도록 정리되었다.
...
|---|---:|---:|---|---|
| 장기 일별 통합 자료 | 11,402 | 5 | 1995-01-01 ~ 2026-03-20 | anomaly block 탐지, 장기 일별 분석 |
...
| anomaly block 결합 자료 | 3,588 | 8 | 1995-01-31 ~ 2026-03-20 | 디커플링 구간의 선행성 분석 |
...
따라서 최종 보고서에서는 고정기간 방식보다 anomaly block 방식을 중심으로 한다. anomaly block은 장기 일별 통합 자료에서 환율과 정책금리차의 30일 이동상관계수를 계산한 뒤, 상관계수가 설정 임계값 이하로 내려가는 구간을 추출하여 구성하였다. 현재 분석 기준은 30일 이동창, 상관계수 임계값 -0.2, 최소 30일 이상 지속되는 구간이다. 이 방식은 금리차와 환율의 관계가 전통적 설명과 달라지는 구간을 데이터 기반으로 탐지한다는 장점이 있다.

현재 설정 기준으로 전체 분석 범위는 1995년 1월 1일부터 2026년 3월 20일까지이며, anomaly block은 총 668개, 전체 anomaly day는 3,588일이다. 첫 anomaly block 시작일은 1995년 1월 31일이고 마지막 종료일은 2026년 3월 20일이다. 이 블록들은 하나의 연속된 사건이 아니라 여러 시점에 흩어진 디커플링 구간이며, 본 연구에서는 이들을 공통된 “금리차 설명력이 약화된 시장 상태”로 보고 결합해 분석한다.

![정상구간과 anomaly block의 상관관계 비교](../analysis/anomaly/correlation_comparison_updated.png)

다만 anomaly block을 이어붙이는 방식에는 주의가 필요하다. 서로 떨어진 날짜를 하나의 시계열처럼 연결하면 실제 달력상의 시간 간격이 사라지고, 직전값 비교나 차분 계산에서 왜곡이 생길 수 있다. 본 프로젝트는 이러한 문제를 인식하고, Granger causality 분석에서는 블록 내부에서만 변화율을 계산하여 블록 경계에서 발생할 수 있는 가짜 점프를 제거했다. 환율 영향 확장 분석에서도 anomaly-only 시계열을 단순히 이어붙이는 방식의 한계를 지적하고, 사건 시점 기준의 local projection 방식으로 보완하려는 시도가 이루어졌다.
...
본 연구는 데이터 수집 및 전처리, 장기 관계 분석, anomaly block 정의, SHAP 기반 요인 분석, M2 구성요소 분석, 단기 유동성 임계점 검증, Granger causality 분석, 예측 모델 검증, 환율 영향 확장 분석의 순서로 진행되었다. 이 흐름은 프로젝트 실행 기록과 각 분석 단계의 결과 요약을 바탕으로 재구성한 것이다.

첫 번째 단계는 데이터 수집 및 전처리이다. 한국은행, FRED, 수동 다운로드 데이터에서 환율, 금리, M2, CPI, 산업생산, KOSPI, 외국인 투자 등 데이터를 수집하고, 전처리 절차를 통해 분석 가능한 시계열 자료로 변환한다. 이 단계에서는 날짜 형식 통일, 계정항목 필터링, 숫자형 변환, 결측치 처리, 월별-일별 빈도 변환이 수행된다.
...
다섯 번째 단계는 시계열 선행성 및 예측 검증이다. Granger causality 분석을 통해 M2 변화가 환율 변화를 선행하는지 확인하고, LSTM 및 Hybrid 모델을 통해 금리차 단독 모델과 단기 유동성 추가 모델의 이상구간 설명력을 비교한다. 마지막으로 환율 영향 분석은 환율 예측 또는 환율 충격이 국내 거시·금융 변수로 어떻게 파급되는지 탐색하는 확장 단계로 배치된다.
...
| anomaly 탐지 | 금리차와 환율의 디커플링 구간 추출 | 이동상관계수 | anomaly block 정의 |
...
상관관계 분석은 환율과 주요 거시 변수 간 선형 관계를 확인하기 위해 사용되었다. 장기 요인 분석 절차에서는 원/달러 환율, 한미 정책금리차, 10년물 금리차, 한국과 미국의 M2, 한국과 미국의 소비자물가, 한국과 미국의 산업생산 등을 같은 표본으로 병합한 뒤 피어슨 상관계수 행렬을 계산하였다. 또한 Random Forest Regressor를 사용하여 환율 수준을 설명하는 변수 중요도를 산출하였다.
...
이 한계를 보완하기 위해 SHAP 분석을 사용하였다. SHAP 분석 절차에서는 정상구간 데이터를 사용해 Random Forest 모델을 학습하고, anomaly period 또는 anomaly block에 해당하는 데이터에 대해 SHAP value를 계산하였다. SHAP은 각 변수가 모델 예측값을 얼마나 상승 또는 하락시키는지 기여도를 계산하는 설명가능 AI 기법이다. 따라서 단순 변수 중요도보다 특정 구간에서 어떤 변수가 환율 상승에 기여했는지 더 세밀하게 해석할 수 있다.
...
![Anomaly block SHAP 요약 그림](../analysis/shap_ml/shap_summary_anomaly.png)
...
일별 SHAP dependence 분석은 MMF와 CMA의 일별 변화량을 설명 변수로 사용하고, 환율 일별 변화량을 목표 변수로 설정한다. 이 분석은 MMF 변화량과 SHAP value 사이의 관계를 시각적으로 확인하여, 특정 수준 이후 MMF의 환율 상승 기여도가 급격히 증가하는지 검토한다. 기존 결과 문서에 따르면 MMF는 특정 수준 이후 SHAP value가 급격히 상승하는 패턴이 관찰된 반면, CMA에서는 같은 패턴이 명확하지 않았다.
...
Granger causality 분석은 anomaly block을 결합한 자료를 사용한다. 이 자료는 각 관측치에 날짜, 환율, 금리차, 한국 M2, 미국 M2, 해당 관측치가 속한 anomaly block의 시작일과 종료일, 블록 내부 순서를 함께 기록한다. 중요한 점은 변화율 계산을 전체 결합 시계열에서 직접 수행하지 않고, 각 블록 내부에서만 계산한다는 것이다.
...
검정은 두 방향으로 수행된다. 첫째, 환율 변화가 M2 변화를 선행하는지 확인한다. 둘째, M2 변화가 환율 변화를 선행하는지 확인한다. 이상구간뿐 아니라 정상구간에서도 같은 검정을 수행하여, 해당 선행성이 일반적 현상인지 아니면 anomaly block이라는 특수한 국면에서 나타나는 현상인지 비교한다.
...
Hybrid 모델은 ARIMA 또는 SARIMAX가 포착하지 못한 잔차 패턴을 LSTM 또는 CNN-LSTM으로 보완하는 구조이다. 본 프로젝트에서는 MMF 기반, 수시입출식저축성예금 기반, 로그수익률 기반 multi-step 예측 실험이 수행되었다. 이 모델들은 환율 예측의 난이도와 단기 유동성 변수의 조건부 설명력을 검토하는 보조 증거로 활용된다.
...
하지만 anomaly block 또는 기존 이상기간 분석에서는 이 관계가 약해지거나 달라졌다. 이상구간 분석 결과, 장기 추세와 달리 이상기간에서 M2와 환율 사이의 선형 상관관계가 매우 낮게 나타났다. 이는 표면적으로는 “M2가 환율을 설명하지 못한다”는 결론처럼 보일 수 있다. 그러나 이 결과는 단기 유동성과 환율의 관계가 선형 상관계수로 포착되기 어려운 비선형 구조일 가능성을 열어준다.
...
SHAP 분석에서는 이상기간 동안 환율 상승에 가장 크게 기여한 변수를 파악하기 위해 Random Forest 모델과 SHAP 기여도 분석을 수행했다. 결과적으로 금리차보다 M2, 특히 단기성 자금이 환율 상승에 크게 기여한 변수로 나타났다. 이 결과는 금리차 중심 설명만으로는 anomaly block의 환율 움직임을 충분히 설명하기 어렵다는 점을 보완한다.
...
일별 임계점 분석은 단기 유동성 데이터, 특히 MMF와 보조적 CMA 정보를 활용해 threshold regression을 수행한 결과를 요약한다. 기존 결과 문서에 따르면 일일 유동성이 약 5.1조 원 이상 증가하는 threshold를 넘는 날 환율이 비선형적으로 크게 점프하는 구조적 변화가 확인되었다. 또한 이 threshold를 돌파한 날이 과거 평균보다 이상기간에 더 자주 발생했다는 결과가 제시되어 있다.
...
일별 SHAP dependence 분석은 MMF와 CMA를 구분하여 해석한다. 결과 문서에 따르면 MMF의 경우 일일 자금 증가분이 특정 수준을 초과하면 환율 상승에 미치는 SHAP value가 가파르게 상승하는 패턴이 관찰되었다. 반면 CMA의 경우 같은 임계점 돌파 패턴이 명확하게 나타나지 않았다. 이는 단기 유동성 중에서도 MMF가 환율 급등 가설을 더 강하게 뒷받침하는 변수일 수 있음을 시사한다.
...
Granger causality 분석은 본 연구의 가장 중요한 검증 중 하나이다. 검정 결과 anomaly block에서 M2 변화가 환율 변화를 선행하는 방향은 통계적으로 유의한 반면, 환율 변화가 M2 변화를 선행하는 방향은 유의하지 않았다. 이는 단기 유동성이 단순히 환율과 함께 움직이는 것이 아니라, 시계열 관점에서 환율 변화보다 먼저 나타나는 정보를 포함할 가능성을 보여준다.
...
정상구간에서는 같은 방향의 관계가 유의하지 않았다. 일반구간에서 환율 변화가 M2 변화를 선행하는 방향의 Lag 2 검정은 p=0.9685였고, M2 변화가 환율 변화를 선행하는 방향의 Lag 2 검정은 p=0.1723으로 유의하지 않았다. 따라서 M2가 환율을 선행하는 구조는 모든 구간에서 일반적으로 작동하는 법칙이라기보다, 금리차 설명력이 약화되는 anomaly block에서 두드러지는 국면 특화적 현상으로 해석하는 것이 적절하다.
...
Anomaly block 전체를 대상으로 한 확장 평가에서는 차이가 더 분명하다. 평가된 block은 546개이며, Model A의 weighted RMSE는 21.16, Model B의 weighted RMSE는 16.62이다. weighted MAE 역시 Model A 19.50, Model B 14.32로 감소했다. 이는 anomaly block에서 MMF 변수가 금리차 단독 모델보다 환율 움직임을 설명하는 데 추가 정보를 제공했음을 보여준다.

수시입출식저축성예금 기반 LSTM에서도 유사한 방향의 결과가 관찰된다. 전체 구간에서는 Model A가 RMSE 44.62, Model B가 RMSE 64.00으로 금리차 단독 모델이 우수했다. 그러나 고정 이상구간에서는 Model A RMSE 21.21, Model B RMSE 18.98로 단기 유동성 추가 모델이 개선되었다. anomaly block 확장 평가에서는 Model A weighted RMSE 15.07, Model B weighted RMSE 14.89로 차이는 작지만 Model B가 더 낮았다.
...
![MMF 추가 모델의 anomaly block 예측 결과](../analysis/LSTM/lstm_mmf/full/anomaly_concatenated_blocks_lstm_plot_full.png)

![수시입출식저축성예금 추가 모델의 anomaly block 예측 결과](../analysis/LSTM/lstm_m2_demand_deposit/full/anomaly_concatenated_blocks_lstm_plot_full.png)
...
최종 환율 영향 분석은 anomaly event month에서 원/달러 환율이 움직였을 때 국내 거시·금융 변수들이 이후 1~6개월 동안 어떤 방향과 크기로 반응하는지 추정한다. 대상 변수는 소비자심리지수, 수입액, KOSPI, 산업생산, 수입물가지수, 외국인 채권투자, 외국인 주식투자, 무역수지 등이다.
...
본 보고서에서는 환율 영향 분석을 핵심 결론이 아니라 확장 가능성으로 다룬다. 핵심은 anomaly block에서 단기 유동성이 환율을 설명한다는 점이며, 환율 영향 분석은 이 환율 변동이 이후 거시경제 변수 예측으로 어떻게 이어질 수 있는지를 보여주는 후속 연구 방향이다. 향후에는 단기 유동성 기반 환율 위험지수와 환율 영향 모델을 결합하여, 환율 급등 가능성과 그 파급효과를 함께 예측하는 조기경보 시스템으로 발전시킬 수 있다.
...
네 번째 한계는 일부 초기 분석 결과와 최신 분석 자료 사이의 정합성 문제이다. 예를 들어 초기 보고서에는 CMA와 MMF를 합친 단기 유동성 대리 지표 분석이 포함되어 있으나, 현재 최종 본문에서 확실하게 설명할 수 있는 일별 자료는 MMF 중심으로 구성되어 있다. 또한 고정기간 이상구간 결과와 anomaly block 결과가 혼재되어 있으므로 최종 제출 전 어떤 결과를 본문 핵심으로 사용할지 정리해야 한다.
...
네 번째 과제는 환율 충격의 거시경제 파급효과 분석 고도화이다. 환율 영향 분석은 환율 충격이 KOSPI, 수입물가, 무역수지, 외국인 투자 등으로 어떻게 전달되는지 탐색한다. 향후에는 단기 유동성 기반 환율 예측과 event-time local projection을 결합하여 “유동성 충격 → 환율 급등 → 국내 거시 변수 반응”의 연쇄 구조를 하나의 시스템으로 모델링할 수 있다.
...
| 한미 정책금리차 | 한국 기준금리에서 미국 정책금리를 뺀 값 | anomaly block 정의, 예측 모델 |
| 한국 광의통화 M2 | 현금, 예금, 단기 금융상품 등을 포함한 통화량 | 장기 분석, Granger 분석 |
...
| MMF | 단기 금융상품에 투자되는 대기성 자금 | 임계점 분석, SHAP dependence, 예측 모델 |
...
| anomaly block 탐지 | 환율과 한미 정책금리차의 30일 이동상관계수를 계산해 디커플링 구간을 추출 | 이상구간 목록과 결합 자료 |
| 장기 요인 분석 | 환율과 주요 거시 변수의 상관관계 및 변수 중요도를 계산 | 상관관계 히트맵, 기준 분석 결과 |
| SHAP 분석 | Random Forest 모델을 이용해 anomaly 구간에서 환율 상승에 기여한 변수를 해석 | 변수별 기여도 그림 |
| M2 구성요소 분석 | M2 내부의 단기성 자금 증가액과 기여도를 계산 | 구성요소별 증가분 표와 그래프 |
| 임계점 분석 | 일별 단기 유동성 증가가 특정 수준을 넘을 때 환율 반응이 달라지는지 검정 | 임계점 회귀 결과 |
| Granger 분석 | M2 변화와 환율 변화의 시간 선후 관계를 양방향으로 검정 | 선행성 검정 결과표 |
...
| SHAP 요약 그림 | anomaly 구간에서 환율 상승에 기여한 변수 | 제4장 제2절 |
| M2 구성요소 증가 그래프 | 단기성 자금의 증가분과 기여도 | 제4장 제3절 |
| Granger causality 결과표 | M2 변화와 환율 변화의 시간 선행성 | 제4장 제5절 |
| 임계점 회귀 그림 | 단기 유동성 증가가 특정 수준을 넘을 때 환율 반응이 달라지는지 확인 | 제4장 제4절 |
| MMF SHAP dependence 그림 | MMF 증가분과 환율 상승 기여도의 비선형 관계 | 제4장 제4절 |
...
2. 초기 임계점 분석은 CMA와 MMF를 합친 대리 지표를 언급하지만, 현재 최종 본문에서 확실하게 설명할 수 있는 일별 자료는 MMF 중심이다. CMA 원천 자료와 최신 파이프라인 반영 여부를 확인해야 한다.
...
5. anomaly block 기준은 30일 이동창과 상관계수 임계값 -0.2 설정을 사용하였다. 최종 보고서 제출 전 이 기준을 연구팀 최종 기준으로 확정해야 한다.
