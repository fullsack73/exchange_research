...
이상 구간이란, 환율과 한-미 정책금리차가 전통적인 음의 상관관계를 보이지 않고 디커플링되는 모든 시점을 의미한다. 일반적으로 금리가 높은 국가로 자본이 유입되어 해당 통화가 강세를 보여야 하지만, 이상 구간에서는 이러한 전통적 메커니즘이 작동하지 않는다.
...
=== 주요 발견
단순 선형 상관관계에서는 유의미성이 보이지 않던 한국 통화량이 SHAP 분석에서는 환율을 비정상적으로 밀어 올린 압도적 1위 원인으로 도출되었다. 이는 통화량과 환율 사이에 단순한 직선의 비례 관계가 아니라, 임계점을 넘어가면 폭발적으로 영향을 미치는 비선형적 관계가 성립함을 시사한다.
