...
def tune_hyperparams(
    ...
    # Cap tuning sample size for speed
    max_tune_train = 1200
    max_tune_val = 300
    ...
    for hidden_dim, num_layers, lr, num_epochs, batch_size in product(*grid.values()):
        model = ExRateLSTM(input_dim, hidden_dim, num_layers)
        criterion = nn.MSELoss()
        optimizer = torch.optim.Adam(model.parameters(), lr=lr)

        train_set = TensorDataset(torch.FloatTensor(x_fit), torch.FloatTensor(y_fit))
...
def train_eval_cpi_integrated():
    ...
    # Prepare data: split 80/20 for train/test
    test_ratio = 0.2
    ...
    for model_idx, (features, model_name) in enumerate([
        ...
        best_params = tune_hyperparams(x_train, y_train, x_train.shape[2], "full_all", model_name)
        print(f"Best params: {best_params}")

        # Train final model on full train set with best params
        model = ExRateLSTM(x_train.shape[2], best_params["hidden_dim"], best_params["num_layers"])
        criterion = nn.MSELoss()
        optimizer = torch.optim.Adam(model.parameters(), lr=best_params["lr"])

        train_set = TensorDataset(torch.FloatTensor(x_train), torch.FloatTensor(y_train))
        train_loader = DataLoader(train_set, batch_size=best_params["batch_size"], shuffle=False)
        train_model(model, train_loader, criterion, optimizer, best_params["num_epochs"])
    ...
    for model_idx, (features, model_name) in enumerate([
        ...
        # Hyperparameter tuning
        best_params_anom = tune_hyperparams(x_train_anom, y_train_anom, x_train_anom.shape[2], "anomaly_period", model_name)
        print(f"Best params: {best_params_anom}")

        # Train final model
        model_anom = ExRateLSTM(x_train_anom.shape[2], best_params_anom["hidden_dim"], best_params_anom["num_layers"])
        criterion_anom = nn.MSELoss()
        optimizer_anom = torch.optim.Adam(model_anom.parameters(), lr=best_params_anom["lr"])

        train_set_anom = TensorDataset(torch.FloatTensor(x_train_anom), torch.FloatTensor(y_train_anom))
        train_loader_anom = DataLoader(train_set_anom, batch_size=best_params_anom["batch_size"], shuffle=False)
        train_model(model_anom, train_loader_anom, criterion_anom, optimizer_anom, best_params_anom["num_epochs"])
