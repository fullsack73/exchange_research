...
def run_one_period(df_period: pd.DataFrame, period_name: str, out_dir: str, enable_tuning: bool = True):
    ...
    features_b = ["USD_KRW", "RATE_SPREAD_KOR_USA", TARGET_M2_COL]
    ...
    # Counterfactual for Model B: freeze demand deposit level in test period
    df_cf = df_period.copy()
    last_train_m2 = train_df[TARGET_M2_COL].iloc[-1]
    df_cf.loc[test_df.index, TARGET_M2_COL] = last_train_m2
    ...
    plt.plot(dates_pred_full, pred_b_full_true, label="Model B (Spread + Demand Deposit)", color="red")
    plt.plot(dates_pred_full, pred_cf_full_true, label="Counterfactual (Flat Demand Deposit)", color="blue", linestyle=":")
    ...
    plt.plot(dates_pred_eval, pred_b_true, label="Model B (Spread + Demand Deposit)", color="red")
    plt.plot(dates_pred_eval, pred_cf_eval_true, label="Counterfactual (Flat Demand Deposit)", color="blue", linestyle=":")
...
def run_experiment():
    ...
    lines = [
        ...
        "- Compare Model A (Spread only) vs Model B (Spread + Demand Deposit)",
        "",
    ]
    for r in results:
