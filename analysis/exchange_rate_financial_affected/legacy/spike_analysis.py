...
plt.savefig('eval/shap_summary_spike_period.png', bbox_inches='tight')
plt.close()
...
feature_importance.to_csv('eval/feature_importance_spike_period.csv', index=False)

# 이벤트 스터디 필터링
event_df = pd.read_csv('full/event_study_results.csv', parse_dates=['release_date'])
event_spike_period = event_df[(event_df['release_date'] >= '2024-11-01') & (event_df['release_date'] <= '2025-12-31')]
event_spike_period.to_csv('eval/event_study_spike_period.csv', index=False)
